...
Species = Species([1 3],:); %种族选择
reduce = 1; %菱形太多则加大reduce，控制增长速度
...
while stop==0
    if run==1
        %根据时间更新extension数据
        Species = double(newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies));
        %生长与打架
...
function [newSpecies] = newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies)
    T = temperatureFunction(dayNumber);
    M = moistureFunction(dayNumber);
    E0 = Species(:,1);
    newExtension = zeros(numberSpecies, 1);
    for i = 1:numberSpecies
        newExtension(i,1) = double(singleNewEctension(E0(i,1),Tadjust,Madjust,M,T,Species(i,3),Species(i,4),SpeciesT(i,1),SpeciesT(i,2)));
    end
    Species(:,1) = newExtension;
    newSpecies = Species;
end
...
function [temperature] = temperatureFunction(day)
    temperature = 20;
end

function [moisture] = moistureFunction(day)
    moisture = 1.8;
end
