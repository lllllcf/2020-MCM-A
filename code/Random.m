...
for rd = 1:round
    for nn = 1
        ...
        Species = [  0.25        0           0.45    3.46     0.2325      0          50     50      0.4828            60            191           4;
                     0.49        0           0.19    4.34     0          0          50     50      0.7852             176            191           4;
                     1.07        0           0.11    1.43     0.001           0          50     50      0.0345            211            135           12;
                     4.71        0           0.1     1.29     0.2847      0          50     50      0              255            231           94;
            
                     1.96        0           0.09    1.28     0.5695      0          50     50      0                73            177           251;
                     6.38        0           0.13    1.68     0.5695      0          50     50      0.09549            251            82           73;
                     10.62       0           0.12    1.31     0.7884      0          50     50      0               255            162           198;
                     10.8        0           0.27    2.81     0.9864      0          50     50      0.3581              44            104           255;
            
                     1.54        0           0.42    1.99     0.4932      0          50     50      0.1008               180            136           253;
                     8.75        0           0.1     1.29     1           0          50     50      0               200            200           200;
                     3.88        0           0.08    1.27     0.8054      0          50     50      0              100            100           100;
                     0.77        0           0.29    5.25     0.4931      0          50     50      1              26            80          1 ];
        %0.1958  2.283
        % low     max
        SpeciesT = [ 6.4    31.1;   %温度上下限
            ...
            19     33.6;
            5.1    33.6 ];
        RESULT = (Species(:,4) - Species(:,3)).*(SpeciesT(:,2) - SpeciesT(:,1));
        %%%%%控制台
        ng = 200; %格子数
        Species = Species([5 9 12],:); %种族选择
        ...
        %菌丝上限和增长公式
        amountLimit = 300; %菌丝上限
        
        %Elo公式的修改
        kG1 = 10; % 进攻方强势时，越大，防御方的防御力越高
        ...
        % 主事件循环
        stop= 0; run = 1;freeze = 0; dayNumber = 0;
        SquareNumber = zeros(3);
        for useless = 1:1200
            if run==1
                %根据时间更新extension数据
                MM = 0;
                TT = 0;
                Species = double(newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies,E0,kind,MM,TT));
                %生长与打架
    ...
    disp(rd);
    nn1 = double(Smatrix == 1);
    nn1 = sum(nn1(:));
    nn2 = double(Smatrix == 2);
    nn2 = sum(nn2(:));
    nn3 = double(Smatrix == 3);
    nn3 = sum(nn3(:));
    ALLSQUARE(1,rd) = nn1 / (nn1 + nn2 + nn3);
    ALLSQUARE(2,rd) = nn2 / (nn1 + nn2 + nn3); 
    ALLSQUARE(3,rd) = nn3 / (nn1 + nn2 + nn3); 
end

hold on
plot(1:round,ALLSQUARE(1,:),'.');
plot(1:round,ALLSQUARE(2,:),'.');
plot(1:round,ALLSQUARE(3,:),'.');
% figure(1);
% pp;
% xlabel('Time(month)');
...
% for q = 1:round*12
% %     if mod(q,2) == 0
% %         plot(1:1000,ALLSQUARE(q,:),'LineWidth',2,'color','[0.984 0.321 0.286]');
% %     else
% %         plot(1:1000,ALLSQUARE(q,:),'LineWidth',2,'color','[0.286 0.694 0.984]');
% %     end
%     plot(1:1000,ALLSQUARE(q,:),'LineWidth',2);
% end
...
function [newSpecies] = newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies,E0,kind,MM,TT)
    T = temperatureFunction(dayNumber,kind,TT);
    M = moistureFunction(dayNumber,kind,MM);
...
function [temperature] = temperatureFunction(day,kind,TT)
    %default
    temperature = 20;
    %a = floor(day/5) + 1;
    %temperature = TT(a,1);
...
function [moisture] = moistureFunction(day,kind,MM)
    %default
    moisture = 1;
    %a = floor(day/5) + 1;
    %moisture = MM(a,1);
