...
while stop==0
    if run==1
        %根据时间更新extension数据
        Species = double(newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies,E0));
        %生长与打架
...
function [newSpecies] = newExtension(dayNumber,Species,SpeciesT,Tadjust,Madjust,numberSpecies,E0)
...
function [moisture] = moistureFunction(day)
    ...
    moisture = 12.3 * cos(2*pi/365*day - pi/2) + 11.75;
    %}
