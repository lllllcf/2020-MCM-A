...
Species = [  0.25        0           0.45    3.46     0.2325      0          50     50      0.4828            60            191           4; 
             0.49        0           0.19    4.34     0.001      0          50     50      0.7852             176            191           4;
             ...
             10.8        0           0.27    2.81     0.9864      0          75     225      0.3581              44            104           255;
             
             1.54        0           0.42    1.99     0.4932      0          25     25      0.1008               180            136           253;
             8.75        0           0.1     1.29     1           0          50     50      0               200            200           200;
             3.88        0           0.08    1.27     0.8054      0          225     225      0              100            100           100; 
             1.77        0           0.29    5.25     0.4931      0          225     75      1              26            80          1 ];
                                    %0.1958  2.283   
         % low     max
...
%%%%%控制台
ng = 300; %格子数
Species = Species([9 9 9 9],:); %种族选择
numberSpecies = size(Species,1); % 种群数目
reduce = 50; %菱形太多则加大reduce，控制增长速度
treeThick = 10000; %木头厚度（mm）
...
while stop==0
    if run==1
        ...
        %Species = double(newPrepare(Species, numberSpecies, omega, L0));
        %生长与打架
        ...
        SquareNumber = [SquareNumber sum(Lmatrix(:))/(treeThick*ng*ng)];
...
hold on;
SquareNumber(1) = [];
plot(1:length(SquareNumber),SquareNumber,'LineWidth',2);
...
function [winRate] = generateWinRate(Species, numberSpecies,Amatrix,battleMatrix,ith,kG1,kL1,ng,kD) % battleMatrix和谁战斗的矩阵
    ...
    for j = 1:numberSpecies
       ...
       xG1 = exp(-1.*kG1.*singleDmatrix2).*singleDmatrix1;
       xL1 = log(kL1.*singleDmatrix2 + exp(1)).*singleDmatrix1;
    ...
    adjustMatrix = double(~(~battleMatrix));
    lambdaMatrix = 2./(1 + 3.*(1/3).^xMatrix).*adjustMatrix;
    winRate = (1./(1 + 10.^(lambdaMatrix.*(rankMatrix - rank0))) - kD.*DeltaMatrix.*log(1 + Amatrix/0.01)).*adjustMatrix;
end
...
function [A] = newPrepare(A0, ns, omega, L0)
    for i = 1:1:ns
        D1 = 12.48*A0(i,1)^0.32; % T是22时
        D2 = 0.82*A0(i,6) + 1.87; 
        J = 122*L0 + 0.5*122*122*A0(i,1);
