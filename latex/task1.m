clear,clc
% 设置GUI按键
plotbutton=uicontrol('style','pushbutton','string','start', 'fontsize',12, 'position',[20,350,50,20], 'callback', 'run=1;');
erasebutton=uicontrol('style','pushbutton','string','stop','fontsize',12,'position',[20,300,50,20],'callback','freeze=1;');
quitbutton=uicontrol('style','pushbutton','string','exit','fontsize',12,'position',[20,250,50,20],'callback','stop=1;close;');
screenNumber = uicontrol('style','text','string','1','fontsize',12, 'position',[20,400,50,20]);

%制造稳定的关键在于 1.充足的食物量  2.引入菌丝量和竞争的正相关关系
%考虑ranking的相互变化

% 初始设置
reduce = 10; %菱形太多则加大reduce
ng = 100; %格子数
speciesChoose = [1 5]; %选择
numberSpecies = length(speciesChoose); % 种群数目
length0 = 0.01; %边长0.01mm
treeThick = 10000; %厚度（mm）
moi = 2.2; %空气湿度
...
Species = [  0.25        0           0.45    3.46     0.2325      0          25     25      0.4828            60            191           4; 
             ...
             4.71        0           0.1     1.29     0.2847      0          75     75      0              255            231           94;
            
             1.96        0           0.09    1.28     0.5695      0          75     75      0                73            177           251;
...
hold on;
for q = 1:numberSpecies
    plot(1:length(SquareNumber),SquareNumber(q,:));
end
