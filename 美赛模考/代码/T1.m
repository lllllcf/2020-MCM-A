            ...
            for i = 1:2:n
                if i == 1
                    if n == 1
                        ...
                        [f1,f2,fr] = tension(0, a1, 100, X^2*lambda/4 ,a2, X^2*lambda/4, rodForce(i, n, F, X, x0),singleRodL(i, n, lambda, X, h0));
                    else
                        [a1,a2] = calPosition(i, n, X, lambda);
                        [b1,b2] = calPosition(i+1, n, X, lambda);
                        [f1,f2,fr] = tension(0, a1, b1, X^2*lambda/4 ,a2, b2, rodForce(i, n, F, X, x0),singleRodL(i, n, lambda, X, h0));
                    end
                elseif i == n
                    if n == 1
                        [a1,a2] = calPosition(i, n, X, lambda);
                        ...
                        [f1,f2,fr] = tension(a1, b1, 100, a2 ,b2, X^2*lambda/4, rodForce(i, n, F, X, x0),singleRodL(i, n, lambda, X, h0));
                    end
                else
                    [a1,a2] = calPosition(i-1, n, X, lambda);
                    [b1,b2] = calPosition(i, n, X, lambda);
...
xlswrite('C:\Users\liche\Desktop\rawData1.xlsx',RESULT);
...
function [fn] = rodForce(i, n, F, X, x0) 
    P = 1000 * 9.8; 
    b = X - x0;
    E = 200; 
    ...
    L = X;
    syms x;
    f(x) = (P*b*x) / (6*E*I*L) * (L^2 - b^2 - x^2);  
    g(x) = (P*b*(-x)) / (6*E*I*L) * (L^2 - b^2 - x^2) + L; 
    
    if ((i-1)/n*X - x0) * (i/n*X - x0) < 0 
...
function [x0, y0] = calPosition(i, n, X, lambda) 
    x0 = (2*i - n - 1) / (2*n) * X + 50;
    y0 = lambda * (x0 - 50)^2;
end
